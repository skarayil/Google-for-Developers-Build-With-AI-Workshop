...
(function() {
  ...
  window.WebSocket = function(url, protocols) {
    ...
    if (inputUrl && isValidUrl(inputUrl)) {
      ...
      let proxyUrl;
      if (apiBase) {
        // If API_URL is https://..., WS should be wss://...
        const wsBase = apiBase.replace(/^http/, 'ws');
        proxyUrl = `${wsBase}/ws-proxy?target=${targetUrl}`;
      } else {
        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const host = window.location.host;
        proxyUrl = `${protocol}//${host}/ws-proxy?target=${targetUrl}`;
      }
      ...
      try {
        ...
        const proxyFetchOptions = {
          ...
          headers: {
            ...
            'X-App-Proxy': 'AnNPcZ3IUqyFLkSEtlDS3UTnjw4iTF72',
          },
          body: JSON.stringify(requestDetails),
        };

        const apiBase = import.meta.env.VITE_API_URL || '';
        console.log(`[Vertex AI Proxy Shim] Fetching from Node.js backend: ${apiBase}/api-proxy`);
        const proxyResponse = await fetch(`${apiBase}/api-proxy`, proxyFetchOptions);
